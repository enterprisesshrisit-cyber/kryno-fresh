...
// ─── CALL BUTTON ─────────────────────────────────────────────────────────────
function CallButton({ icon, type }: { icon: any; type: 'voice' | 'video' }) {
...
export default function ChatScreen({ route, navigation }: any) {
  ...
  const sendMessage = () => {
    ...
    const newMsg = {
      id: `msg${Date.now()}`, from: 'me' as const,
      text: input.trim(), time: 'now', reactions: [],
    };
    ...
    setTimeout(() => {
      ...
      setMessages(prev => [...prev, {
        id: `msg${Date.now() + 1}`, from: 'them' as const,
        text: 'That resonates deeply ✦', time: 'now', reactions: [],
      }]);
      setTimeout(() => flatRef.current?.scrollToEnd({ animated: true }), 80);
    }, 2800);
  };
      ...
      <SafeAreaView edges={['top']} style={{ flex: 1 }}>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.headerActions}>
            <CallButton icon="call-outline"     type="voice" />
            <CallButton icon="videocam-outline" type="video" />
