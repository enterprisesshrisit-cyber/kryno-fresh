...
import { COLORS, FONTS, RADIUS, SPACE, TIER, MOOD, STATUS, type MoodType } from '../lib/theme';
import { FEED_POSTS, STORIES, ME } from '../lib/data';
...
function StoryBubble({ story }: { story: any }) {
  ...
  return (
    <TouchableOpacity onPress={press} activeOpacity={1} style={styles.storyWrap}>
...
// ─── FEED CARD ───────────────────────────────────────────────────────────────
function FeedCard({ post, focusMode }: { post: typeof FEED_POSTS[0]; focusMode: boolean }) {
    ...
    <View style={styles.root}>
      ...
      <SafeAreaView edges={['top']} style={{ flex: 1 }}>
        <Animated.FlatList
          ...
          renderItem={({ item }) => <FeedCard post={item} focusMode={focusMode} />}
